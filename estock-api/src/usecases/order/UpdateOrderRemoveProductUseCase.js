...
class UpdateOrderRemoveProductUseCase {
    async process(orderId, inputData) {
        ...
        UpdateOrderRemoveProductUseCase.#validateOrderId(id);

        const order = await orderDataProvider.getOrderById(id, excludeFilter, {});

        UpdateOrderRemoveProductUseCase.#validateOrderSetNewValue(order, inputData);
        await new Validator().validate(order);
        UpdateOrderRemoveProductUseCase.#setOrderTotalAndEconomy(order);

        return orderDataProvider.updateOrderRemoveProduct(order, ["createdAt", "updatedAt", "user"], selectsOptions);
    }
...
module.exports = UpdateOrderRemoveProductUseCase;